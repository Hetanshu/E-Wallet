...
public class TransactionService {
    ...
    public void updateTransaction(String message) throws JsonProcessingException{
        ...
        if(status=="SUCCESS") transaction.setStatus(TransactionStatus.SUCCESS);
    ...
    private void callNotificationService(Transaction transaction) {
        ...
        String senderMessage=String.format("Hi %s, Your transaction with transactionId %s to %s of amount %d is in %s status.",
                senderName,transaction.getTransactionId(),receiverName,transaction.getAmount(),transaction.getStatus());
        ...
        if(transaction.getStatus()==TransactionStatus.FAILED) return;
